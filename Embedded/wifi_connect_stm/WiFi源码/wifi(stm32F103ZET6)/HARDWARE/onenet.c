...
#define AUTH_INFO	"lian"

#define DEVID		"12345678"
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(!json){
					UsartPrintf(USART_DEBUG,"Error before---------\r\n");
						while(OneNet_DevLink())			//接入OneNET
						delay_ms(500);
					OneNet_Subscribe(ch, 1);
				}
					else{
					json_value=cJSON_GetObjectItem(json,"LED");//解析：键值
					if(json_value->type==cJSON_Number){
						UsartPrintf(USART_DEBUG,"get LED---> %d\r\n",json_value->valueint);
