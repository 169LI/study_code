...
#define PROID			"9hu56lk2XL"

#define ACCESS_KEY		"dTl5a214Y2xhTlQ4TXVGNTZ1YWpneHNTUTdyMEpLSlg="
...
//----------------------------------------------------加密-------------------------------------------------------------------------
	...
	UsartPrintf(USART_DEBUG, "hmac_sha1_buf");
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"wendu\":{\"value\":%d},", 27);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"shidu\":{\"value\":%d},",70);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"switchs\":{\"value\":%s}",0?"true":"false");
	strcat(buf, text);
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				params_json =cJSON_GetObjectItem(raw_json,"params");//提取这个键
				LED_json 		=cJSON_GetObjectItem(params_json,"switchs");//提取这个键
				
				if(LED_json!=NULL){
					UsartPrintf(USART_DEBUG, "data=----- %d\r\n",LED_json->type);
					if(LED_json->type == cJSON_True)	LED0=0; // 灯亮
					else LED0=1;//灯灭
				}
				cJSON_Delete(raw_json);//释放内存；
