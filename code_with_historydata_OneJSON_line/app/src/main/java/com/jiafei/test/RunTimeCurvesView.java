...
public class RunTimeCurvesView extends View {
    ...
    private float axisXStepValue = 2;
    ...
    private void calCurvesParam(float currentValue) {
        // 判断是否所有曲线的值都相同
        boolean allValuesSame = checkAllValuesSame();

        if (allValuesSame) {
            // 如果所有值相同，则设置一个默认范围，这里将默认范围设置为当前值加减 5
            int defaultRange = 5;
            allCurvesMinValue = currentValue - defaultRange;
            allCurvesMaxValue = currentValue + defaultRange;
            axisYStepValue = currentValue/10; // 默认步进值为1
        } else {
            // 计算实际的最大值和最小值
            ...
            for (MCurve curve : mCurves.values()) {
                for (float value : curve.values) {
                    ...
                    //allCurvesdValue =allCurvesMaxValue - allCurvesMinValue;
                }
            }

            if(allCurvesMinValue<=12){
                allCurvesMinValue =0;
                allCurvesMaxValue=14;
                axisYStepValue=1;
            }else{
                axisYStepValue=allCurvesMaxValue/14;
            }
        }
    }
    private void calCurvesParamPm(float currentValue) {
        // 判断是否所有曲线的值都相同
        boolean allValuesSame = checkAllValuesSame();
        ...
        if (allValuesSame) {
            ...
            allCurvesMinValue = currentValue - defaultRange;
            allCurvesMaxValue = currentValue + defaultRange;
            axisYStepValue = currentValue/14; // 默认步进值为1
        } else {
            // 计算实际的最大值和最小值
            allCurvesMinValue = allCurvesMaxValue = currentValue;
    ...
    private void calCurvesParamHum(float currentValue) {
        // 判断是否所有曲线的值都相同
        boolean allValuesSame = checkAllValuesSame();
    ...
    private boolean checkAllValuesSame() {
        if (mCurves.isEmpty()) {
            return false;
        }

        float firstValue = mCurves.values().iterator().next().values.peek();
        for (MCurve curve : mCurves.values()) {
            for (float value : curve.values) {
                if (value != firstValue) {
                    return false; // 只要有一个值不同，就说明不是所有值都相同
                }
            }
        }
    ...
    private void drawXYCoordinateUnit(Canvas canvas) {
        ...
        float x_text_y = getHeight() - getPaddingBottom() - mBottomTextHeight / 2 + offsetY+30;
        float x_text_x = getRight() - mTextPaint.measureText(mXUnit)-25;
    ...
    private void drawXCoordinateMetric(Canvas canvas) {
        ...
        for (int i = 0; i < mCoordinateXCount; i++) {
            ...
            Paint.FontMetricsInt fontMetrics = mTextPaint.getFontMetricsInt();
            float offsetY = ((fontMetrics.bottom - fontMetrics.top) / 2 + fontMetrics.bottom) / 2;
            float baseLine = getHeight() - getPaddingBottom() - mBottomTextHeight / 2 + offsetY-30;
...
//                values.removeFirst();
    ...
    public boolean push2CurvePm(int id, float data) {
        if (mCurves.keySet().contains(id)) {
            ConcurrentLinkedQueue<Float> values = mCurves.get(id).values;
            if (values.size() >= mXDivNum) {
//                values.removeFirst();
    ...
    public boolean push2CurveHum(int id, float data) {
        if (mCurves.keySet().contains(id)) {
            ConcurrentLinkedQueue<Float> values = mCurves.get(id).values;
            if (values.size() >= mXDivNum) {
//                values.removeFirst();
    ...
    public void ClearOn(boolean isc) {
        this.isc = isc;
        init();
    }
}
